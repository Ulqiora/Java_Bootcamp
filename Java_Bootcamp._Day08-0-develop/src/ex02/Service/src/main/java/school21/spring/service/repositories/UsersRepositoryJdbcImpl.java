...
public class UsersRepositoryJdbcImpl implements UsersRepository {
    ...
    public List<User> findAll() {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            while (rs.next())
                users.add(new User(rs.getLong(1), rs.getString(2),rs.getString(2)));
        } catch (SQLException e) {
    ...
    public void save(User entity) {
        try (Connection connection = dataSource.getConnection();
             Statement statement = connection.createStatement()) {
            int result = statement.executeUpdate("INSERT INTO models.user (id, email, password) VALUES ("
                    + entity.getId() + ", '"
                    + entity.getEmail() + ", '"
                    + entity.getPassword() + "');");
    ...
    public void update(User entity) {
        String upQuery = "UPDATE models.user SET email = '"+entity.getEmail()+"', '"+entity.getPassword()+"' WHERE id = "+entity.getId();
        try (Connection connection = dataSource.getConnection();
             Statement statement = connection.createStatement()) {
