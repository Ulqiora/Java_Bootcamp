...
public class Transaction {
    ...
    public Transaction(User recipient, User sender, TransferCategory type, Integer amount) {
        ...
        try {
            ...
            else if(type == Transaction.TransferCategory.Debits&&(sender.getBalance()<amount))
                throw new IllegalArgumentException("Sender balance more than amount");
            else if(type == Transaction.TransferCategory.Credit&&(recipient.getBalance()<amount))
                throw new IllegalArgumentException("Recipient balance more than amount");
            Integer resultRec = recipient.getBalance() + ((type == Transaction.TransferCategory.Debits) ? (1) : (-1)) * amount;
            if (resultRec < 0)
                throw new IllegalArgumentException("Illegal argument");
